(function initWegoApp() {
  ...
  // 移动端视口高度兜底：iOS 键盘收起后 100dvh 可能不恢复，导致底部出现占位
  (function initViewportHeightManager() {
    ...
    function updateHeight() {
      ...
      if (!h || h === cachedHeight) return;
      cachedHeight = h;
      phoneFrame.style.height = h + 'px';
      phoneScreen.style.height = h + 'px';
    }
    ...
    // iOS 键盘收起后 visualViewport.resize 偶尔不触发，focusin/focusout 兜底
    document.addEventListener('focusin', function () { setTimeout(scheduleUpdate, 300); });
    document.addEventListener('focusout', function () { setTimeout(scheduleUpdate, 300); });
