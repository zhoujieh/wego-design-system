...
5. 读取 `experience/candidates.json`，按 `normalizedKey` 去重；同类问题只合并证据和场景。
6. 将新增或更新后的候选写回唯一数据源。
7. 向用户输出阶段总结，由用户选择：`升级`、`继续观察` 或 `不沉淀`。

不得因为候选数量、出现次数或时间经过自动升级。"全部升级"等批量指令必须逐条按本流程处理，不构成跳过分类与候选池的快捷路径。
