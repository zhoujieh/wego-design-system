...
若规则落点为 `.codex/skills/shared/references/design-decisions.md`，只允许承载跨场景的顶层原则与页面判断：产品输入、设计输入、组件选择、布局和视觉取舍、状态交互或设计系统缺口判断。业务事实、合同字段、组件内部结构、页面范式、运行时实现、资源消费和测试方法必须分别落到其对应权威源，不能以设计决策原则文档汇总代替。
