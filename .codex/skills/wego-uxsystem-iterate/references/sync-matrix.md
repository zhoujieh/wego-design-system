...
| 收口轻沉淀（一次性教训） | `.codex/skills/wego-uxsystem-iterate/experience/evidence.json`、`.codex/skills/wego-uxsystem-iterate/experience/EXPERIENCE.md` | — | 用 `refine-experience.mjs --related` 取相关事实；同一任务同根因只计一次；摘要用 § 分隔并引用存在的 evidence ID；跑 `--check`；随当前业务 PR 进 main |
...
| 迭代已有场景技能 | 目标 `wego-scene-*/SKILL.md` | `.codex/skills/wego-uxsystem-iterate/experience/EXPERIENCE.md`（次数/日期） | 单技能 ≤1500 字符；成熟后回流 references 并删除自身 |
...
`.codex/skills/wego-uxsystem-iterate/experience/evidence.json` 保存不可删除的事实事件；`.codex/skills/wego-uxsystem-iterate/experience/EXPERIENCE.md` 由 AI 基于事实推理维护，用 § 分隔一句话摘要（≤1500 字符），每条必须追溯到事实事件；固定流程细节毕业到 `.codex/skills/wego-scene-*` 场景技能。`refine-experience.mjs --check` 仅做事实一致性校验、格式检查和场景技能校验，不生成内容，`--related` 为只读查询。
