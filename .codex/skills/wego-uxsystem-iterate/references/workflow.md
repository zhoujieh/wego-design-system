...
- 页面范式、允许组件、核心/支持组件、UI Kit：`uikit-plan.json` 与 `ui_kits/*`。
...
4. root、必需节点、alternatives、modifier 必须都在 Preview 和聚合 CSS 中可达。替代结构使用结构化 `alternatives`，不得保留不可解析的复杂 selector 字符串。
5. 更新组件索引、允许组件、核心/支持清单、页面蓝图和 Preview 导航；新增或删除组件时必须同步所有清单。
6. 运行 CSS 提取、组件一致性守卫、资源同步和受影响场景回归。

## 3. UI Kit 迭代步骤

1. UI Kit 仅展示页面组合，不得成为 App 宿主、通用组件或业务场景模板。
2. 每个 UI Kit 只能使用已注册组件与业务区域胶水；不得使用旧技能路径、自动生成规则文档、未注册 class 或自造组件。
3. 更新 pagePattern、fallback blueprint、presentation、componentPlan、允许组件和质量报告；报告必须使用当前 schema 和设计系统版本，记录通过的组件一致性检查与非空质量门禁。
4. 无法覆盖的结构创建 DDR，选用最近似正式结构；禁止因缺口在 UI Kit 或场景临时造组件。
...
Token、组件、Preview、UI Kit 或消费契约改变时递增 `metadata.json.version`。守卫失败必须修正源文件后重跑，不能用 DDR 记录已知实现错误。
