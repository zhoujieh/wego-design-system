...
读取 `AGENTS.md`、[原型基线](references/high-fidelity-prototype-baseline.md)、[组件与 UI Kit 迭代](references/workflow.md)、[资源同步矩阵](references/sync-matrix.runtime.md)、[工作流与经验同步](references/sync-matrix.md)、[工作流迭代](references/workflow-iteration.md)、[经验候选](references/experience-candidates.md)、[场景设计决策方法](../wego-design/references/design-decisions.md)、归属注册表和候选池。组件或 UI Kit 改动必须读取其契约、Preview、`components.css`、索引和受影响明确页面范式。

## 输出契约与跨技能交接

输出权威源改动、组件一致性守卫结果、必要资源同步和 DDR 决策。新增或修改组件、Preview、UI Kit、Token 或消费规则时，必须同步索引、引用该组件的明确范式、质量报告与版本；只有用户确认的经验才可升级正式规则。
