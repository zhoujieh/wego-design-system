...
输出权威源改动、必要同步和与改动范围相称的验证结果。组件、Token、Preview 或 UI Kit 变更如有可视 Preview，交付时从当前 PR head 分支 worktree 启动本地 HTTP 服务，返回已核实且可点击的对应预览链接，不返回 GitHub Pages 预览链接；对未合并 PR，先核对承载服务的 worktree 正处于该 PR 的 head 分支且预览内容包含本次改动。本地验收服务在等待用户验收期间保持运行，只在用户明确验收通过、任务废弃、PR 关闭或合并、worktree 即将清理，或被同一交付单元的新服务替换时停止并删除记录。收到设计系统缺口时先验证缺口，再采用正式能力或明确回退。正式设计系统源变化必须递增版本；只有用户确认升级的经验才修改正式能力。经验升级后保留事实、累计次数和升级历史；升级后再次出现时立即重新进入调整。守卫验证可观察的源码、结构化数据或运行结果，不校验文档固定措辞。
