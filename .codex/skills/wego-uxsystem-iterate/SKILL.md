...
输出权威源改动、组件一致性守卫结果、必要资源同步和缺口处理结论。收到最小缺口说明时，先验证缺口真实存在，再记录采用的正式能力或回退；不建立平行状态机。新增或修改组件、Preview、UI Kit、Token 或消费规则时，严格按资源同步矩阵只同步该变更类型命中的权威源、生成物、消费者、质量证据和部署副本；正式设计系统源变化必须递增版本，不得把未受影响的索引、范式或质量报告一律改写。只有用户确认的经验才可升级正式规则。
