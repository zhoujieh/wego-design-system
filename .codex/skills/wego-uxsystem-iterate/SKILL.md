...
输出权威源改动、必要同步和与改动范围相称的验证结果。组件、Token、Preview 或 UI Kit 变更如有可视 Preview，交付时返回对应本地预览文件的绝对路径，不返回 GitHub Pages 预览链接。收到设计系统缺口时先验证缺口，再采用正式能力或明确回退。正式设计系统源变化必须递增版本；只有用户确认升级的经验才修改正式规则。守卫验证可观察的源码或运行结果，不校验文档固定措辞。
