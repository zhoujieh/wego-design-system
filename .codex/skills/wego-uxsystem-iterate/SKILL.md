...
- 组件、Token、Preview 或 UI Kit 变更如有可视 Preview，从当前任务 worktree 启动或复用本地 HTTP 服务，并通过临时 HTTPS 隧道返回已核实且可远程查看的本地预览链接；不返回 GitHub Pages 预览链接。
...
- 允许按一组相关改动创建本地 checkpoint commit；小问题先本地累计，不为每个细节推送。组件、Token、Preview 或 UI Kit 完成一轮后默认只交付一个可远程查看的本地预览链接；只有用户明确要求推送 PR、更新 PR、在线预览，或工作流维护短周期收口时，才由 `wego-github-delivery` 推送远端分支并创建/更新同一 PR；在线预览核实后关闭对应本地预览与隧道。
...
3. 组件、Token、Preview 或 UI Kit 仍只交付可远程查看的本地预览；工作流和守卫维护不要求在线预览。
4. 设计系统变更等待用户明确验收通过后合并；工作流权威源维护免业务验收，必要检查通过后可按短周期 PR 自动合并。
5. 每次结果标明：`当前状态：已验收，合并中（PR #<编号>）`；工作流维护已自动合并时改为报告合并结果。

"改好了""继续""再调整一下"等普通反馈不构成验收授权，也不构成推送 PR 授权。PR 已存在时，后续小问题先回到本地迭代累计并交付可远程查看的本地预览；只有用户明确要求更新 PR/在线预览时才集中更新原 PR，不重复创建。

本地预览服务和临时 HTTPS 隧道在本地迭代和等待用户查看期间保持运行；用户要求 PR/在线预览且在线部署核实通过后、任务废弃、PR 关闭或合并、worktree 即将清理，或被同一交付单元的新服务替换时停止并删除记录。正式设计系统源变化必须递增版本；收到设计系统缺口时先验证缺口，再采用正式能力或明确回退。只有用户确认升级的经验才修改正式能力。经验升级后保留事实、累计次数和升级历史；升级后再次出现时立即重新进入调整。守卫验证可观察的源码、结构化数据或运行结果，不校验文档固定措辞。
