...
- 允许按一组相关改动创建本地 checkpoint commit；小问题先本地累计，不为每个细节推送。到达交付节点（首次可验收版本、一组反馈已集中处理完成、用户明确要求更新 PR/在线预览，或工作流维护短周期收口）并通过本地验证后，由 `wego-github-delivery` 推送远端分支并创建/更新同一 PR。
...
2. 确认 PR 已包含全部本次交付节点改动；工作流权威源维护直接创建/更新短周期 PR。
...
"改好了""继续""再调整一下"等普通反馈不构成验收授权。PR 已存在时，后续小问题先回到本地迭代累计；到达下一次交付节点时再集中更新原 PR，不重复创建。
