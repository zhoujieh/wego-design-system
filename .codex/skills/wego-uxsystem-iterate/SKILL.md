...
- 修改业务迭代契约、状态机、范围确认、开发交接、冻结或跨技能阶段守门。
...
- 跨技能仓库硬约束：`AGENTS.md`。
- 技能触发条件、职责边界、必要输入、正式输出契约和跨技能交接：对应技能唯一的 `SKILL.md`。
- 技能内部判断方法、详细步骤、检查清单、例外处理和案例：对应技能 `references/*.md`，并由 `SKILL.md` 明确引用。
...
经验正式升级时默认不得修改任何 `SKILL.md`。只有本次规则直接改变以下任一入口契约时，才允许修改对应技能的 `SKILL.md`：
...
修改前必须在本轮归属判断中明确写出命中的白名单项、变化前后的契约和下游影响。无法命中其中任一项时，禁止修改 `SKILL.md`，应改写到结构化权威来源、已有 reference、脚本或测试。

以下内容不得以“补充说明”“最佳实践”“注意事项”“新增检查项”或“经验沉淀”为由追加到 `SKILL.md`：

- 单个组件或页面的经验。
- 具体业务案例和 Bug 修复过程。
- 详细判断方法、长检查清单和同步矩阵。
- 适用场景、反例、例外和回退的完整分析。
- 已存在于组件 JSON、Preview、UI Kit、Token 或消费配置中的规则正文。
- 可以由脚本或测试执行的检查逻辑。

行数限制只是补充守门，不能作为允许继续向 `SKILL.md` 追加内容的依据。
...
- `SKILL.md` 只保留入口契约与精简导航并少于 500 行；详细 schema、兼容迁移、判断方法、长示例和专项检查放入一层 `references/`。
...
- 结构守门必须检查 frontmatter、目录名、入口行数、直接链接、资源角色、agents 元数据和 `SKILL.md` 写入白名单；基础格式校验通过不能替代这些检查。
...
- 一次审查可以记录多条经验候选，但每条都必须完成归属判定、运行时可达性验证和场景拆分。
- 入池前必须确认问题最早产生的环节、主要归属、次要归属、权威落点、运行时消费方和验收方式。
- 归属不明确时不得入池。
- 同类经验复用已有候选，只增加 `occurrence_count` 和场景证据，不重复新增。
- 当前处于快速迭代阶段，新候选和本轮复用更新的候选默认阈值为 1；标准稳定阶段阈值保留为 3，后续只调整阈值配置即可恢复。
- 候选达到自身 `threshold` 时将状态置为 `awaiting-confirmation`，并提示用户：
...
4. 先执行 `SKILL.md` 五项白名单判断，未命中时不得修改任何 `SKILL.md`。
5. 确认正式权威落点、运行时消费者、输出字段、下游传播和验收方式。
6. 修改最早权威来源，并同步所有必需投影。
7. 增加对应回归测试或守门。
8. 涉及设计系统本体时递增版本。
9. 生成并检查人工规则文档。
10. 将候选改为 `promoted`，记录正式文件、字段和规则 ID。
...
- 业务迭代工作流变化是否同步 `wego-product` 权威契约、四阶段技能、AGENTS、状态机脚本、生成规则来源和回归测试；普通业务技能不得自行改写迭代状态机。
