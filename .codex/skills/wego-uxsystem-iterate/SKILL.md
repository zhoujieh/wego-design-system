...
用于 `wego-design` 设计系统和工作流的正式迭代。目标只有两个：
...
- 迭代模式 / 审查模式 → 读 `references/workflow.md`（步骤 + 组件迭代读取顺序 + 浏览器核对）、`references/sync-matrix.md`（同步范围，不含工作流迭代场景）、`references/judgment-principles.md`（组件级判断原则 + 组件级守门）
