...
需求（自然语言 / 参考图 / Figma）
  ...
  → wego-design 本地迭代，先提供可远程查看的本地预览
  → 用户持续反馈小问题，默认继续本地累计并返回可远程查看的本地预览
  → 用户明确要求 PR/在线预览时，再推送并切换为同一 PR 的在线预览
...
分支、worktree、预览和 PR 由 `wego-github-delivery` 统一管理。新任务默认创建或复用工作分支并进入本地迭代；完成一轮实现或反馈处理后，默认只启动/复用本地预览，并通过临时 HTTPS 隧道返回一个可远程查看的本地预览链接；只有用户明确要求推送 PR、更新 PR 或在线预览时，才推送并更新同一 PR，同时等待部署完成后只返回在线预览链接并关闭对应本地预览与隧道；合并进 `main` 必须用户确认终局材料。它不再是第四条业务主链路。各技能只按当前任务读取各自入口直接引用的权威来源。
