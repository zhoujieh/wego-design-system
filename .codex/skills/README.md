...
原始需求
  ...
  → prototype_brief 草案
  → 必须生成会话线框
  → submit-brief 绑定当前范围与线框生成凭据
  → 共同展示简报与线框
  → 用户确认 prototype_brief
  → wego-design
```

- `wego-product` 先遵循共享设计决策原则，再确认业务事实、范围、入口、关键路径、状态、数据、原型边界和产品阶段交互视觉描述，形成完整 `prototype_brief` 草案；随后必须用当前宿主能力生成对应参考线框，无渲染器时降级为文本分镜。
- 会话线框用于帮助用户确认当前 `prototype_brief`；反馈先更新简报，再重新生成线框。线框不保存合同或独立确认状态，也不替代用户对简报的明确确认。
- `wego-design` 把已确认 `prototype_brief` 作为设计授权，先完成设计意图、精确范式/自主组合裁决和语义区域树，再进行 Preview-first 组件映射、场景实现、一轮设计自审、决策提取与既有检查。
