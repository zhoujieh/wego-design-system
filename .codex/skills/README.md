...
- `wego-product` 先遵循共享设计决策原则，再确认业务事实、范围、入口、关键路径、状态、数据、原型边界和产品阶段交互视觉描述；需求改变用户可见结构且最小事实齐备时，用当前宿主能力生成参考线框，无渲染器时降级为文本分镜。
- 会话线框只帮助需求探索，结论归纳进现有 `prototype_brief`；不保存线框合同，也不替代用户确认。
