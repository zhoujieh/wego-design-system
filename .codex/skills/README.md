...
分支、worktree、预览和 PR 由 `wego-github-delivery` 统一管理。新任务默认创建或复用工作分支并进入本地迭代；完成一轮实现并通过本地验证后**自动推送并更新同一 PR**，同时返回本地与在线两个链接；合并进 `main` 必须用户确认终局材料。它不再是第四条业务主链路。各技能只按当前任务读取各自入口直接引用的权威来源。
