...
需求（自然语言 / 参考图 / Figma）
  ...
  → 用户持续反馈小问题，本地累计到交付节点再更新同一 PR 和在线预览
...
分支、worktree、预览和 PR 由 `wego-github-delivery` 统一管理。新任务默认创建或复用工作分支并进入本地迭代；小问题先本地累计，到达交付节点并通过本地验证后再推送并更新同一 PR，同时返回本地与在线两个链接；合并进 `main` 必须用户确认终局材料。它不再是第四条业务主链路。各技能只按当前任务读取各自入口直接引用的权威来源。
