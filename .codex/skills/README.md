...
同一需求复用当前未冻结迭代；简报变化按产品技能重新提交并确认。设计系统缺口由 `wego-design` 提交最小说明，交给 `wego-uxsystem-iterate`。分支和 PR 由 `wego-github-delivery` 按已识别的交付单元复用或收口，它不是第四条业务主链路。各技能只按当前任务读取各自入口直接引用的权威来源。
