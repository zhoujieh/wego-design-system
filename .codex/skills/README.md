...
本文件只负责帮助 agent 判断当前请求应触发哪个技能。详细规则以各技能入口文件为准。
...
| 正式生成或更新 `wego-app` 场景 | `wego-ux` | 两份规格已落盘 | 完成后交给 `wego-tests` |
...
- 没有 `page_spec` 不进入 `wego-design`。
- 没有 `design_consumption_plan` 不进入 `wego-ux`。
