...
每个业务需求都必须先整理极简 `prototype_brief`，内容只包括：目标、纳入范围、排除范围、入口、关键路径、原型边界、业务状态、数据合同、低风险假设和待确认问题。产品阶段可在这些字段中附带交互视觉描述（布局位置、控件类型、视觉强调等），由 `wego-design` 严格遵循；正式规格（具体组件名、CSS 类、Token、动画名）仍由设计阶段在设计系统范围内决定。
...
- **规范不允许**：产品阶段的视觉描述与设计系统顶层原则（`design-decisions.md` 的清晰 > 高效 > 一致 > 美观裁决）、Token 语义、Preview-first 组件消费或组件契约冲突时，设计系统优先；冲突项记录到场景决策证据，并退回 `wego-product` 重新确认。
