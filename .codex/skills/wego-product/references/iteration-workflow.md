...
确认时所有相关 `interaction_spec.readiness` 必须为 `ready` 或 `ready-with-assumptions`。第一版不把 `partially-ready` 迭代整体交给设计；未确认内容应明确延期或排除。

## 7. 各阶段所有权

### wego-design

- 只在 `product-confirmed` 后工作。
