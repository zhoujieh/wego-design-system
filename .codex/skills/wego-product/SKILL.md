---
name: "wego-product"
description: 创建并确认业务迭代，将原始中文需求转换为跨场景可追踪、可验证、可交接的 interaction_spec。用于新页面、新原型、新场景、业务流程或已有场景的目标、范围、状态、异常流程、宿主入口发生变化时；在任何设计系统消费和原型实现之前使用。不要用于纯组件、Token、UI Kit、实现偏差或验收任务。
---

# Wego Product

负责创建业务迭代、确认跨场景产品范围并落盘 `interaction_spec`，不选择 UI Kit、组件、布局或实现方式。
...
- 正式业务变化必须先用 `scripts/iteration-record.mjs init` 在唯一主业务场景创建迭代，区分长期 `feature_id` 与本轮 `requirement_id`，并登记所有关联场景和宿主文件。
- 每个相关 `interaction_spec` 必须包含同一 `iteration_context`；生成范围确认文档并获得用户明确确认后，通过脚本进入 `product-confirmed`，否则不得交接。

## 执行流程

1. 判断是新需求、业务变更、实现偏差，还是设计系统/工作流问题。
2. 为业务需求创建迭代，确定主场景、关联场景、`feature_id`、`requirement_id` 和需要持续比较的共享场景基线范围。
3. 提炼目标、用户、范围、完成条件和交互模式。
4. 拆分流程节点、surface、内容块、状态、异常与数据交接。
5. 判断宿主 Tab、入口层级、场景目录和稳定 route，并登记场景外运行时文件。
6. 区分已确认事实、低风险可逆假设和待确认问题。
7. 按 [interaction-spec.md](references/interaction-spec.md) 更新所有相关场景规格，并建立需求到 flow/node/surface/result 的追踪。
8. 运行 `iteration-record.mjs scope` 生成范围确认文档，获得用户明确确认后运行 `confirm-scope`。
9. 归档旧版、落盘新版并交给 `wego-design`。
...
迭代总表保存到主业务场景 `_iterations/{YYYYMMDD}-{iteration_id}-{title}/iteration.json`；范围确认文档使用 `{iteration_id}-{title}-范围确认.md`。完整结构、状态机和跨场景规则以 [业务迭代契约](references/iteration-workflow.md) 为准。
