...
当原始需求描述的是仓库、商品、员工、客户、门店等“业务对象列表”，且页面目标包含新增、编辑、删除、启停或进入详情时，`page_spec` 需要先做字段分级，再交给 `wego-design` 决定布局：

- `list_required_fields`：列表必显字段，用于识别对象和关键状态，例如对象名称、启用状态、主图/头像
- `list_summary_fields`：列表摘要字段，只保留 1-2 条影响当前判断的信息
- `detail_fields`：完整地址、联系人、备注、配置明细、长文本等详情/编辑字段，不默认摊在列表里
- `operation_fields`：新增、编辑、删除、启停、更多等动作，标明高频/低频/危险动作

这些分级只能来自用户原始需求，不得为了凑列表样式新增业务字段。输出时可写入 `component_intent`，或在对应 `page_surfaces[].purpose` 里说明字段分级和列表页职责。
