...
- ⑦ iconfont：入口仅在选中已注册图标时出现；纯图标网格可滚动，移动端固定约 42dvh 且至少四行、不参与 flex 压缩；同组件结构且原图标一致时整组同步；修改记录、撤销/重做、删除、刷新回放整组闭环。[ev-055, ev-056, ev-059]
- ⑧ 工具栏：桌面固定屏幕水平居中、位于可见 bottom-nav 上方 16px 且不可拖动；居中不得用 transform 破坏 fixed 子面板坐标；配置列表/日志须完整可见，数据模拟/更多锚定各自入口；移动端保留原逻辑；收起态有配置显示数字、无配置显示走查箭头，展开态收起入口显示叉图标；全部工具入口 Tooltip 完整。[ev-057, ev-058, ev-059]
