...
description: 走查工具分层测试。改动 walkthrough-tool.js/css 后先跑冒烟和受影响项；本地可验收版本跑专项闭环；终局或 PR 发布验证跑双端关键链路。用于 walkthrough 测试、回归、验收、异常排查。
---
...
## 固定流程
0. 定层级：本地小改=冒烟+受影响项；本地可验收版本=本地专项闭环；终局/PR 发布=双端关键链路。
...
4. 本地可验收版本才验回显+撤销/重做+刷新持久化；终局再扩到全部关键项。
5. 证据：轻量层记操作+结果+时间戳；交付/终验层补 DOM+必要 localStorage。

## 交付前检查清单
- 层级匹配：小改不过度全量；本地可验收版本覆盖受影响闭环；终局/PR 发布覆盖双端关键链路。
- 基础冒烟：入口、选中、面板打开、主控件回显、无页面报错。
- 颜色/HSL/渐变改动：按 ①② 验回显；本地可验收版本再验撤销/重做/刷新持久化。
- 数值/顺序/连点/元信息改动：按 ③④⑤⑥ 验受影响路径；本地可验收版本补边界和持久化。
