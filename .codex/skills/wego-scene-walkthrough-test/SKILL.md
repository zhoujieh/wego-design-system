...
- ② 渐变：色标加（≤5/≥2）/删/拖位；±45° 与滑块调角度；填充/文本支持、描边/投影不支持；入口已收敛颜色选择器。[ev-047]
...
## 踩坑反例
- 回归脚本须用当前交付单元分支上的技能脚本（.codex/skills 随分支不同步），禁止跑主仓库 main 旧版（含已移除断言误报 FAIL）。[ev-052]
- 恢复滚动会异步派发 scroll，同步标志无效，须时间窗/临时移除监听器。[ev-051]
- 渐变撤销：切渐变+改色标各一条，回 none 须撤 2 步。[ev-050]
- 顺序移动后原坐标命中兄弟元素须重取；一次移动=一个撤销单元（moveKey）。[ev-042, ev-048]
- 冒烟须覆盖移动按钮同档换位路径（曾漏测作用域 bug）。[ev-045]
- 无背景元素 fillHex 为空、HSL 改色不生效属既定行为，勿判失败。[ev-043]
