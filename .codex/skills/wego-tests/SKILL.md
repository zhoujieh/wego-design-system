...
- 是否具备 Vercel 静态部署条件
- 是否存在 `wego-app/.vercel/project.json` 或等价项目绑定结果
- 本轮是否执行了自动部署并产出主入口 URL
- 若部署失败，是否明确降级到本地入口 `wego-app/index.html`

## 部署结果验收

- 默认检查 `deployment_readiness_check.project_created_or_reused` 是否记录为 `created` 或 `reused`
- 默认检查 `deployment_readiness_check.project_binding` 是否记录了 `wego-app/.vercel/project.json` 或等价项目绑定信息
- 默认检查 `deployment_readiness_check.deploy_attempted=true`，除非本轮被明确中止在部署前
- 默认检查 `deployment_readiness_check.deploy_status`
- 当 `deploy_status=success` 时，必须有 `entry_url`
- 当 `deploy_status=degraded-local` 时，必须有失败原因，且 `fallback_local_entry` 必须为 `wego-app/index.html`
- 默认不强制点击线上链接，只检查部署结果与主入口链接字段是否完整
