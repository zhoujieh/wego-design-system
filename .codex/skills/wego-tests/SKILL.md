---
name: "wego-tests"
description: 验收已经生成并注册 route 的 wego-app 业务场景，比较 interaction_spec、design_plan、正式规则来源和真实实现，输出 acceptance_report。用于验收、回归、review、浏览器交互检查、布局检查和交付就绪判断；不要在场景未生成时使用，也不要把验收问题直接沉淀为正式工作流规则。
---
...
4. `rule_sources_used` 指向的正式组件、pagePattern、fallback、Token 和 Preview。
5. 当前 scene、路由、宿主、资源和真实交互状态。
6. 构造或校验报告时读取 [acceptance-report.md](references/acceptance-report.md)。
7. 验完整业务路径、兼容迁移或专项规则时读取 [acceptance-checks.md](references/acceptance-checks.md)。
8. 做布局、键盘、交互或线上验证时读取 [browser-verification.md](references/browser-verification.md)。
...
1. 校验两份规格、readiness、surface、route 和设计缺口。
