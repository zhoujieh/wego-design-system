...
部分验收项已被守门脚本（`validate-wego-design.mjs`）覆盖为硬门禁，无需人工重复检查。未覆盖的项仍需人工验收。

| 验收项 | 硬门禁 | 守门函数 |
|--------|--------|----------|
| _spec 文件存在 | 是 | checkPrototypeSurfaceDesigns |
| route_id 在 routes.js 注册 | 是 | checkPrototypeSurfaceDesigns |
| scene.js 调用 registerScene | 是 | checkPrototypeSurfaceDesigns |
| surface_designs 覆盖 page_surfaces | 是 | checkPrototypeSurfaceDesigns |
| 外壳类不泄漏 | 是 | checkPrototypeShellLeakage |
| _spec 归档 | 是 | checkPrototypeSpecArchive |
| Token 不硬编码 | 是 | checkPrototypeTokenHardcoding |
| 组件类不发明 | 是 | checkPrototypeComponentClassInvention |
| navbar leftControl 绑定 | 否 | 人工验收 |
| 组件 DOM 结构与契约一致 | 否 | 人工验收 |
| 业务流程与 page_spec 一致 | 否 | 人工验收 |

守门硬门禁不需要手动复查，人工验收时只需关注未覆盖项。
