...
5. `.design_library/wegoux/components/index.json`
6. `.design_library/wegoux/components/{slug}.json`
7. `.design_library/wegoux/preview/component-{slug}.html`

按任务继续补读：

- 涉及 Token：读 `.design_library/wegoux/colors_and_type.css` 和 `.design_library/wegoux/css.json`
- 涉及 UI Kit：读 `.design_library/wegoux/ui_kits/app/index.html` 和对应 `quality-report.json`
- 涉及图标：读 `.design_library/wegoux/iconfont.css`、`.design_library/wegoux/assets/fonts/`、`.design_library/wegoux/assets/icons/`
- 涉及规范或文案：读 `.design_library/wegoux/specs/` 对应文件
...
- 禁止直接手改 `.design_library/wegoux/components.css`
- 组件样式优先改 `.design_library/wegoux/preview/component-{slug}.html` 中 `/* @component-css-start */` 与 `/* @component-css-end */` 之间的样式
...
- 涉及 Token 时，必须同步 `.design_library/wegoux/css.json`
- 修改 `.design_library/wegoux/css.json` 后，必须执行 `python3 -c "import json; json.load(open('.design_library/wegoux/css.json'))"`
- 组件样式有变动时，必须在仓库根执行 `node .design_library/wegoux/scripts/extract-components-css.mjs .design_library/wegoux`
- 正式迭代必须递增 `.design_library/wegoux/metadata.json` 的 `version`
...
- `.design_library/wegoux/components/index.json` 是已发布组件权威来源
- `navbar-action-button` 是嵌入式组件，不要误当成独立预览页组件
- `button` 案例参考读 `references/button-example.md`
- 若用户要求直接改 `.design_library/wegoux/components.css`，应明确拦截并改走 preview 标记区 + 提取脚本流程
