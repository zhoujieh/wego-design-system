...
- 如果真实浏览器核对被工具能力阻塞，要如实记录阻塞原因，并用静态扫描、JSON 校验、生成脚本、资源引用检查做降级验证；不要把自动化问题误记成组件问题。
- 需要真实浏览器核对时，先确认本地静态服务 `HEAD 200`，再优先用当前 agent 自带内置浏览器；若当前 agent 不支持或出现 attach / webview 失败，再切换到 Playwright。
...
- 绝对定位的错误文案、浮层提示、辅助说明不能默认写死 `white-space: nowrap`；必须同时检查长中文文案在 320~390px 宽度内是否会换行、截断或溢出，并把约束写进契约。
- 如果 preview 里的交互脚本移除了某个状态 class 的切换逻辑，例如 `is-focus`、`is-active`，必须同步检查契约里的 `variantDimensions`、`representativeVariants`、`stateClasses` 和对应 CSS 是否还在承诺该状态；不能留下“契约存在、预览永远触发不到”的死状态。
...
- 如果要做真实浏览器核对，是否已经按“静态服务探活 → 当前 agent 自带内置浏览器 → Playwright”准备验证链路，而不是继续使用 `file://`
- 如果有真实浏览器核对，是否已经在移动视口下至少检查一轮截图和横向溢出，而不是只看桌面宽度
- 如果本轮把提示文案、气泡、错误信息改成绝对定位，是否已经用一条偏长中文文案检查过换行和横向溢出
...
- 需要浏览器核对时，可先在 `wegoux/` 根目录执行 `python3 -m http.server 4173`，再访问 `http://127.0.0.1:4173/preview/component-{slug}.html` 或 `ui_kits/...`；不要继续使用 `file://`
- 需要浏览器核对时，固定顺序是：`http.server` 探活 → 当前 agent 自带内置浏览器 → Playwright。
