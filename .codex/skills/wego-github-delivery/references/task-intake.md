...
├─ 无 → 走完整启动（第 1~6 步）
└─ 有 → 新需求属于当前上下文吗？
     ├─ 属于（同一迭代覆盖场景、同一业务目标的延伸）→ 跳过启动，直接继续
     └─ 不属于（换场景、换业务目标）→ 走轻量启动（第 1~6 步；第 2 步至少 fetch，第 5 步按需）
```

**"属于当前上下文"的判断标准**：当前迭代的主场景、关联场景或 `affected_scenes` 已覆盖该请求，且业务目标不变（迭代 ID 不变）。未被当前迭代覆盖的场景或不同业务目标，才是独立交付单元。
...
- **matched**：接手结果指定的分支、worktree 和可选开放 PR。已提交简报范围内的反馈留在原迭代中处理；只有改变业务事实时才回到 wego-product 更新简报。
...
- **new**：以第 2 步 fetch 后的最新 `origin/main` 为基创建 `feature/<owner>-<task>` 分支 + 独立 worktree（`git worktree add ../<owner>-<task> -b <分支> origin/main`）。创建完成前不得写迭代、spec 或业务文件。
- **matched**：切换到已有 worktree，确认当前分支与任务分支一致。
- 确认 worktree 中无未提交的游离改动（归属不清的先交由用户确认保留或还原）。

## 第 5 步：需求规格确认（仅业务原型新需求或范围变化）
...
- 切换交付单元时走轻量启动（第 1~6 步；第 2 步至少 fetch，第 5 步按需）。
