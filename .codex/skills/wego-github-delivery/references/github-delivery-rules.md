...
- 有可视目标时启动或更新本地 HTTP 预览；默认只返回本地链接，用户明确要求 PR/在线预览且已推送 PR 后才返回本地与在线两个链接；无可视入口时只报告验证结果。
...
PR 已存在时，用户继续提出小问题，回到本地迭代累计修改并优先返回本地预览；只有用户明确要求更新 PR/在线预览时才集中更新同一个 PR。只有用户确认终局材料才合并。
...
业务原型默认只返回本地预览；用户明确要求 PR/在线预览并已推送 PR 后，或进入合并阶段时，必须返回两个指向同一 routeId 或目标入口、且均包含本次改动的链接：
...
- GitHub Pages 产物根即应用入口（`previews/pr-<编号>/`，无 `wego-app/` 前缀）。业务原型只有在用户明确要求 PR/在线预览且已推送 PR 后，或合并阶段才等待并核实该地址；设计系统仍不得把 Pages 部署当作完成条件。
