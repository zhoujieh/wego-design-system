...
- 一个交付单元固定一个工作分支和 worktree，同一需求固定复用同一个 PR，最多只有一个开放 PR；PR 只在交付节点创建或更新，不随每个细节改动自动推送。
...
- 有可视目标时启动或更新本地 HTTP 预览；尚未到交付节点时只返回本地链接，已推送 PR 的交付节点才返回本地与在线两个链接；无可视入口时只报告验证结果。
...
- 只有到达交付节点才推送远端分支并创建/更新对应 PR：首次可验收版本、一组反馈已集中处理完成、用户明确要求更新 PR/在线预览，或多人协作需要远端同步。同一需求固定复用同一个 PR，不重复创建；不得为每个细节制造推送。推送前先让本地与改动相称的验证通过（源码检查、关键交互检查，必要时 `--scope=changed`），禁止未验证直接推送；`--scope=full --strict` 完整静态验证保留到合并阶段。
- 本地预览与在线 PR 预览不视为提交授权或合并授权，仅用于沟通当前进度。
- 每次结果标明：`当前状态：本地迭代中（已推送，PR #<编号>）`；尚未推送或 PR 与预期不符时标明`当前状态：本地迭代中（未推送）`并说明原因。

### 提交与合并授权门禁

- **交付节点推送不需要额外授权**：用户已确认立项/修改方向后，AI 可在交付节点推送并创建或更新同一个 PR；普通本地小改动、单次回复结束、checkpoint commit 不构成交付节点。
...
多人或多 Agent 确有远端协调需要时，可仅推送用于暴露分支的最小协调提交；是否创建 PR 取决于是否已到交付节点，不得把后续本地小改动自动推送；执行前必须说明这是并发协调例外。
...
PR 已存在时，用户继续提出小问题，回到本地迭代累计修改；到达下一次交付节点时再集中更新同一个 PR。只有用户确认终局材料才合并。
...
业务原型在已推送 PR 的交付节点和合并阶段必须返回两个指向同一 routeId 或目标入口、且均包含本次改动的链接；尚未推送的本地累计阶段只返回本地预览：
...
- GitHub Pages 产物根即应用入口（`previews/pr-<编号>/`，无 `wego-app/` 前缀）。业务原型只有在已推送 PR 的交付节点和合并阶段才等待并核实该地址；设计系统仍不得把 Pages 部署当作完成条件。
