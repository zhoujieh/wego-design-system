...
- 本地预览记录：worktree、branch、target、可空 PR 编号、隧道进程和远程本地预览 URL。
...
- 一个交付单元固定一个工作分支和 worktree，同一需求固定复用同一个 PR，最多只有一个开放 PR；PR 只在用户明确要求推送 PR、更新 PR、在线预览，或多人协作需要远端同步时创建或更新，不随完成一轮实现或每个细节改动自动推送。
...
- 有可视目标时启动或更新本地 HTTP 预览，并通过临时 HTTPS 隧道生成可远程查看的本地预览；默认只返回该远程本地预览链接。用户明确要求 PR/在线预览且已推送 PR 后，只返回在线预览链接并关闭对应本地预览与隧道；无可视入口时只报告验证结果。
...
- 只有用户明确要求推送 PR、更新 PR、在线预览，或多人协作需要远端同步时，才推送远端分支并创建/更新对应 PR；首次可验收版本、一组反馈已集中处理完成、单次回复结束、checkpoint commit 或 Agent 自行判断“完成”都不触发推送。同一需求固定复用同一个 PR，不重复创建；不得为每个细节制造推送。推送前先让本地与改动相称的验证通过（源码检查、关键交互检查，必要时 `--scope=changed`），禁止未验证直接推送；`--scope=full --strict` 完整静态验证保留到合并阶段。
- 远程本地预览与在线 PR 预览不视为提交授权或合并授权，仅用于沟通当前进度。
- 每次结果标明：`当前状态：本地迭代中（已推送，PR #<编号>）`；尚未推送或 PR 与预期不符时标明`当前状态：本地迭代中（未推送）`并说明原因。

### 提交与合并授权门禁

- **推送 PR 需要明确请求**：用户确认立项、修改方向、查看本地预览或继续反馈都不构成推送授权；只有用户明确要求“推 PR”“更新 PR”“给在线预览”“发布在线预览”等远端动作，AI 才可创建或更新同一个 PR。
...
多人或多 Agent 确有远端协调需要时，可仅推送用于暴露分支的最小协调提交；是否创建 PR 取决于用户是否需要在线预览或 PR 协作入口，不得把后续本地小改动自动推送；执行前必须说明这是并发协调例外。
...
PR 已存在时，用户继续提出小问题，回到本地迭代累计修改并优先返回可远程查看的本地预览；只有用户明确要求更新 PR/在线预览时才集中更新同一个 PR，在线部署核实后切换为单一在线预览链接并关闭对应本地预览与隧道。只有用户确认终局材料才合并。
...
- 本地 HTTP 地址只作为承载地址，不直接作为交付链接；默认使用 Cloudflare Tunnel 等临时 HTTPS 隧道生成一个可远程查看的本地预览链接。
- 业务原型远程本地预览链接指向同一应用入口和 routeId；设计系统使用与改动对应的 Preview 或 UI Kit 入口。
- 返回链接前确认 worktree 当前分支与任务分支一致，实际请求远程本地预览 URL 为 HTTP 200，并确认目标内容包含本次改动。

<!-- rule-id: business-preview-links-must-be-paired -->
### 业务预览单链接切换

业务原型同一交付阶段只返回一个验收链接，避免本地和在线入口并存造成验收口径分叉：

- 未推送或未要求在线预览：只返回远程本地预览链接，核对承载服务的 worktree 当前分支与任务分支一致，并验证 HTTP 200 和目标内容。
- 用户明确要求 PR/在线预览并已推送 PR 后，或进入合并阶段时：只返回 `https://<owner>.github.io/<repo>/previews/pr-<编号>/` 加同一目标 hash 的在线预览链接；必须实际请求核实 HTTP 200，并确认 `publish` 检查成功、部署 SHA 与该次 workflow 事件的 `GITHUB_SHA` 一致、目标产物包含本次改动。pull_request 事件通常使用 merge ref SHA，不得强行与 PR head SHA 比较。
- 在线预览核实通过后，关闭该交付单元对应的本地 HTTP 服务和临时 HTTPS 隧道并清理记录；后续同一 PR 有新改动时，重新进入本地迭代预览，直到用户再次要求更新 PR/在线预览。
- 目标阶段的唯一链接无法核实时必须说明阻塞，不得返回另一阶段链接并声称交付完整。

设计系统组件、Token、Preview 或 UI Kit 不使用 GitHub Pages 作为验收入口，即使已创建 PR，仍只交付对应远程本地预览。
...
本地迭代阶段的预览和隧道都属于受管服务，不属于必须在单次回复结束前退出的临时验证服务：

- 在 `.tasks/preview-servers/` 记录端口、PID、worktree 绝对路径、分支、阶段（`local-iteration`）、可空 PR 编号、目标入口、远程本地预览 URL、隧道 PID 和启动时间；记录不得提交。
- 同一交付单元已有服务时，只有本地进程存活、远程本地预览 URL 可访问、worktree 与分支仍匹配才可复用；不一致时先停止旧进程与旧隧道、删除旧记录，再启动新服务。
- 从本地迭代进入在线预览阶段时，不复用原服务作为交付入口；在线预览核实通过后停止本地服务和隧道。
- 回复结束、对话结束或等待用户查看期间不得关闭仍有效的服务。
- 用户明确要求推送 PR/在线预览且在线部署核实通过后、用户确认终局材料并进入冻结合并、明确废弃任务、任务分支或 worktree 即将清理、PR 关闭或合并，或服务被同一交付单元的新服务替换时，必须停止服务与隧道并删除记录。
- 新任务开始时扫描服务记录，清理进程已失效、隧道失效、worktree 已不存在、分支不匹配，或已关联且 PR 已关闭/合并的残留服务；没有 PR 但仍处于有效本地迭代的服务不得清理。
- 用户继续调整且使用同一 worktree、分支和端口时复用原服务和隧道，修改后重新请求远程本地预览 URL，确认内容已更新。
...
- GitHub Pages 产物根即应用入口（`previews/pr-<编号>/`，无 `wego-app/` 前缀）。业务原型只有在用户明确要求 PR/在线预览且已推送 PR 后，或合并阶段才等待并核实该地址；核实通过后只交付该在线链接并关闭对应远程本地预览。设计系统仍不得把 Pages 部署当作完成条件。
