...
3. 合并前再次同步 `main` 并复验，同时兜底确认 `.tasks/experience-inbox.json` 已完成经验收口扫描（空则已明示无信号），通过后合并 PR 进 `main` 并完成收口；未获用户明确验收通过绝不合并。
...
- **交付前校验**：本地迭代阶段核对服务 worktree 当前分支与任务分支一致；合并阶段还必须核对与 PR head 分支一致。交付链接前实际请求确认 HTTP 200 且目标入口包含本次改动。**在线链接返回必须等部署完成**：推送后先确认 `gh pr checks <N>` 的 publish 通过，再 curl 校验 `https://zhoujieh.github.io/wego-design-system/previews/pr-<N>/` 部署产物（如 `.wego-deployment-sha` 与期望 sha 一致、入口资源 200），两者就绪后才返回在线链接；在线部署通常需数分钟，未就绪时明确告知等待状态并补齐，不得只交付本地链接。
