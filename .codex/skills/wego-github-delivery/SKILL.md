...
- 允许修改、轻量源码检查、关键交互检查和按一组相关改动创建本地 checkpoint commit；不得为每个细节推送或更新 PR。完成一轮实现、首次可验收版本或一组反馈集中处理完成后，默认仍只返回本地预览链接；只有用户明确要求推送 PR、更新 PR、在线预览或多人协作需要远端同步时，才推送远端分支并创建/更新同一个 PR；推送前先让与改动相称的本地验证通过（源码检查、关键交互检查，必要时 `--scope=changed`），禁止未验证直接 `git push`。
- 有可视内容时，从当前任务 worktree 启动或复用本地 HTTP 预览；仅在用户明确要求 PR/在线预览且已推送 PR 后返回在线预览链接，且必须等部署完成后再返回。无可视入口的工作流或文档维护直接报告验证结果。
...
PR 已存在时，用户继续提出小问题仍先回到本地迭代，多个反馈先本地累计并先返回本地预览；只有用户明确要求更新 PR/在线预览时才集中更新同一个 PR。只有用户确认终局材料才合并。
...
- 当前交付单元已提交 checkpoint 但用户尚未要求 PR/在线预览时，允许登记为挂起继续本地迭代；不得仅因一次回复结束、首次可验收版本或一组反馈完成就推送或创建 PR。用户已明确要求 PR/在线预览但尚未推送时，必须推送创建/更新 PR，或显式说明挂起原因。
