...
- 允许修改、轻量源码检查、关键交互检查和按一组相关改动创建本地 checkpoint commit；不得为每个细节推送或更新 PR。完成一轮实现、首次可验收版本或一组反馈集中处理完成后，默认只返回一个可远程查看的本地预览链接；只有用户明确要求推送 PR、更新 PR、在线预览或多人协作需要远端同步时，才推送远端分支并创建/更新同一个 PR；推送前先让与改动相称的本地验证通过（源码检查、关键交互检查，必要时 `--scope=changed`），禁止未验证直接 `git push`。
- 有可视内容时，从当前任务 worktree 启动或复用本地 HTTP 预览，再用临时 HTTPS 隧道生成可远程查看的本地预览链接并只返回该链接；用户明确要求 PR/在线预览且已推送 PR 后，只返回在线预览链接，且必须等部署完成后再返回，同时关闭对应本地预览与隧道。无可视入口的工作流或文档维护直接报告验证结果。
...
PR 已存在时，用户继续提出小问题仍先回到本地迭代，多个反馈先本地累计并先返回可远程查看的本地预览；只有用户明确要求更新 PR/在线预览时才集中更新同一个 PR，部署通过后切换为单一在线预览链接并关闭对应本地预览与隧道。只有用户确认终局材料才合并。
...
- 当前交付单元已提交 checkpoint 但用户尚未要求 PR/在线预览时，允许登记为挂起继续本地迭代；不得仅因一次回复结束、首次可验收版本或一组反馈完成就推送或创建 PR。用户已明确要求 PR/在线预览但尚未推送时，必须推送创建/更新 PR，或显式说明挂起原因；在线预览可用后关闭该交付单元对外的本地预览与隧道。

## 预览服务

- **启动与复用**：从当前交付单元 worktree 根目录启动，仅绑定 `127.0.0.1`；端口、PID、worktree、分支、阶段、可空 PR 编号、目标入口、隧道 PID 和远程本地预览 URL 记录在 `.tasks/preview-servers/`（记录不得提交）。同一交付单元已有可用且信息一致的服务和隧道时复用，否则先停止旧服务和旧隧道再启动新服务；进入合并阶段前若已切换为在线预览，不保留远程本地预览。
- **交付前校验**：本地迭代阶段核对服务 worktree 当前分支与任务分支一致，并实际请求远程本地预览 URL 确认 HTTP 200 且目标入口包含本次改动；合并阶段还必须核对与 PR head 分支一致。**在线链接返回必须等部署完成**：推送后先确认 `gh pr checks <N>` 的 publish 通过，再 curl 校验 `https://zhoujieh.github.io/wego-design-system/previews/pr-<N>/` 部署产物；`.wego-deployment-sha` 应与该次 workflow 事件的 `GITHUB_SHA` 一致，pull_request 事件通常是 merge ref SHA，不能强行与 PR head SHA 比较。入口资源和目标内容核实后只返回在线链接；在线部署通常需数分钟，未就绪时明确告知等待状态并补齐，不得用远程本地预览替代在线链接。
- **停止时机**：服务和隧道在本地迭代、回复结束、对话结束、终局材料准备和等待用户确认期间继续保留；用户明确要求推送 PR/在线预览且在线部署核实通过后、用户确认最终材料并进入冻结合并、明确废弃任务、任务分支/worktree 即将清理、PR 关闭或合并，或服务被同一交付单元的新服务替换时，必须停止服务和隧道并删除记录。
...
- 不把远程本地预览、完成修改、通过检查、创建 checkpoint commit 或用户查看页面视为提交授权或合并授权。
- 不删除 `main`、当前正在使用的分支、带 `keep-branch` 标签的分支，或有未提交改动的 worktree。
- 不停止状态仍为本地迭代、且与记录 worktree 和分支一致的本地预览服务与隧道；切换为已核实的 PR 在线预览后除外。
