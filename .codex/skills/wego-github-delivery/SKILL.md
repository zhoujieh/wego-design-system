...
- 允许修改、轻量源码检查、关键交互检查和按一组相关改动创建本地 checkpoint commit；不得为每个细节推送或更新 PR。只有到达交付节点（首次可验收版本、一组反馈已集中处理完成、用户明确要求更新 PR/在线预览，或多人协作需要远端同步）时，才推送远端分支并创建/更新同一个 PR；推送前先让与改动相称的本地验证通过（源码检查、关键交互检查，必要时 `--scope=changed`），禁止未验证直接 `git push`。
- 有可视内容时，从当前任务 worktree 启动或复用本地 HTTP 预览；仅在已推送 PR 的交付节点返回在线预览链接，且必须等部署完成后再返回。无可视入口的工作流或文档维护直接报告验证结果。
...
PR 已存在时，用户继续提出小问题仍先回到本地迭代，多个反馈先本地累计；到达下一次交付节点时再集中更新同一个 PR。只有用户确认终局材料才合并。
...
- 当前交付单元已提交 checkpoint 但尚未到达交付节点时，允许登记为挂起继续本地迭代；不得仅因一次回复结束就推送或创建 PR。已到交付节点但尚未推送时，必须推送创建/更新 PR，或显式说明挂起原因。
