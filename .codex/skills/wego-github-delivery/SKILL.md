...
2. 运行交付单元核对，不得只检查当前目录或 `main`：
   - **范围**：覆盖全部 worktree、开放 PR 和未冻结迭代；业务任务用场景匹配，非业务任务用明确的改动范围。
   - **开放 PR 落后**：逐个检查开放 PR 分支相对 `main` 的落后情况；落后分支先 merge 同步最新 `main`（含冲突解决与生成物再生成）再继续，等待合并的 PR 同样适用，落后超过 20 个提交时先向用户报告。
   - **本地分支归属**：用 `git branch -vv` 盘点全部本地分支；凡非 `main`、非当前任务、无开放 PR、无挂起登记的分支均为悬空交付单元，先向用户报告并收口（推送创建 PR、登记挂起任务或确认废弃删除），不得静默跨会话保留。
...
5. 核对出现多个候选或无法读取必要信息时，停止并向用户说明阻塞原因；不得自行选择或继续创建迭代。
