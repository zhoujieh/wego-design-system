...
- 完成每轮修改后运行与改动相称的源码检查、关键交互检查和真实浏览器验证。
- 从当前任务 worktree 启动或复用本地 HTTP 预览，确认目标 routeId 包含本次改动，只返回本地链接。
- 保持迭代状态为 `prototyping`；不得自动执行 `submit-prototype`、推送分支、创建或更新 PR，也不得等待在线预览。
- 每次结果标明：`当前状态：本地迭代中（未推送）`。

只有用户明确要求提交、推送、创建/更新 PR、发在线预览，或明确说明“这一轮改完了，可以验收”时，才进入**正式验收**：

1. 运行完整源码守卫和真实浏览器检查。
2. 执行 `submit-prototype` 固定待验收指纹，将迭代推进到 `awaiting-prototype-confirmation`。
3. 集中提交并推送当前功能分支，创建或更新同一个 PR。
4. 同时提供本次任务同一 routeId 或目标入口的两个链接：已核实的本地 HTTP 预览，以及已核实的当前 PR 独立在线预览。任一链接未确认包含本次改动时不得宣称正式交付完成。
5. 每次结果标明：`当前状态：正式验收中（PR #<编号>）`。

“改好了”“继续”“再调整一下”等普通反馈不构成提交授权。即使已有 PR，后续小问题仍先在本地累计修改；只有用户再次明确要求更新 PR，才批量推送。

- 用户在 `prototyping` 阶段要求调整视觉、布局、组件、Token、路由或交互时，直接在原迭代继续修改，不执行 `invalidate --stage=prototype`。
- 用户在 `awaiting-prototype-confirmation` 阶段提出上述调整时，先执行 `invalidate --stage=prototype`，回到本地迭代完成修改；继续复用同一 worktree、分支和本地服务。再次获得提交授权后重新 `submit-prototype` 并更新原 PR。
