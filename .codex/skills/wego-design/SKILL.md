...
- 产品阶段临时线框由 `prototype_brief` 派生，只用于用户确认，不进入设计输入链；即使仍存在于同一会话上下文，也只能消费用户已确认的 `prototype_brief`，不得反向从线框补造事实、组件和视觉规格或机械照搬其外观
...
- 设计系统无法支持已确认的产品阶段交互视觉描述，且正式回退会改变已确认产品指令或用户可见结果时，停止实现，记录冲突与正式回退，再退回 `wego-product`；更新简报、重新生成线框并确认后才能实现
