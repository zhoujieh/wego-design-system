...
- 完成每轮修改后运行与改动相称的源码检查、关键交互检查和真实浏览器验证，通过后由 `wego-github-delivery` **自动推送远端分支并创建/更新同一 PR**；同一需求固定复用同一个 PR。
- 从当前任务 worktree 启动或复用本地 HTTP 预览，确认目标 routeId 包含本次改动，只返回本地链接。
- 保持迭代状态为 `prototyping`；不得自动执行 `submit-prototype`，也不得等待在线预览。
- 每次结果标明：`当前状态：本地迭代中（已推送，PR #<编号>）`；尚未推送时标明`当前状态：本地迭代中（未推送）`。

只有用户明确提出验收意图（要求正式验收、发在线预览，或明确说明“这一轮改完了，可以验收”）时，才进入**正式验收**：

1. 运行完整源码守卫和真实浏览器检查，确认自动推送的 PR 已包含全部本次改动。
2. 执行 `submit-prototype` 固定待验收指纹，将迭代推进到 `awaiting-prototype-confirmation`。
3. 复用并核对现有 PR，不再重复创建或推送。
4. 同时提供本次任务同一 routeId 或目标入口的两个链接：已核实的本地 HTTP 预览，以及已核实的当前 PR 独立在线预览。任一链接未确认包含本次改动时不得宣称正式交付完成。
5. 每次结果标明：`当前状态：正式验收中（PR #<编号>）`。
...
- 用户在 `awaiting-prototype-confirmation` 阶段提出上述调整时，先执行 `invalidate --stage=prototype`，回到本地迭代完成修改；继续复用同一 worktree、分支和本地服务。用户再次明确表达验收意图后重新 `submit-prototype` 并更新原 PR。
- 用户明确验收通过后执行带当前迭代 ID 的 `confirm-prototype --user-confirmed-prototype`；若验收提交发生任何漂移，必须重新提交验收。确认成功后先停止对应本地验收服务并删除服务记录，再同步最新 `main`、解决冲突并完成验证，最后合并到 `main`。
