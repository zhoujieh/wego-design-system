...
本文件是 `wego-design` 技能的唯一运行时权威入口。只负责把已确认并落盘的 `page_spec` 转换为可追溯、可执行、可验收的 `design_consumption_plan`。

## 何时触发

- 已有完整 `page_spec`，需要选择页面范式、UI Kit、fallback blueprint、布局、组件组合或打开方式。
- 已有场景发生组件映射、页面结构或展示方式偏差，需要重新生成设计计划。
- 用户明确要求基于现有需求规格做设计系统消费。

不要误用：

- 没有已确认并落盘的 `page_spec`：回到 `wego-product`。
...
- `wego-app/scenes/{中文业务场景}/_spec/page_spec.json` 存在且结构完整。
- `page_spec.open_questions` 中不存在会改变页面范围、核心流程或宿主路径的未决问题。
- `task_type = design-system-consumption`。其他类型按 `wego-product` 的交接规则转走。
- `page_spec.app_target`、`host_container`、`route_id` 和 `page_surfaces` 能互相对应。
...
2. 已落盘的 `page_spec.json`：唯一业务需求输入。
...
- 设计判断只能来自已确认的 `page_spec`、正式页面范式、Token、组件契约、Preview 和消费边界，自动生成文档不得参与决策。
...
- `design_consumption_plan` 必须落盘后才能交给 `wego-ux`，且不得输出 `spec_refs_used`。

## 设计决策流程

1. 校验 `page_spec` 输入和所有 surface。
...
`surface_designs` 必须覆盖 `page_spec.page_surfaces[]` 的每个 `id`：
...
- 每个 surface 的信息块必须来自对应 `page_spec.page_surfaces[].information_blocks`。
...
- `block`：对应 `page_spec.information_blocks` 的标识。
...
当主 surface 管理一组可新增、编辑、删除、启停或进入详情的业务对象，且 `page_spec` 已完成字段分级时：
...
`page_spec.host_container.leaf_level >= 3` 时，若使用 push，必须在说明中声明栈式导航需求。场景级页面默认 `covers_tab_bar = true`；只有明确属于宿主内嵌 `host-entry` 时可为 `false`。
...
`design_consumption_plan.app_target` 必须从 `page_spec.app_target`、`route_id` 和 `host_container` 映射：

- `mode = wego-app-scene`
- `app_root = wego-app`
- `scene_folder` 与 `page_spec` 完全一致
- `route_id` 与 `page_spec.route_id` 完全一致
- `route_mode = hash`
...
`wego-app/scenes/{中文业务场景}/_spec/design_consumption_plan.json`

规则：

- 必须在 `page_spec.json` 已落盘后生成。
- 最新版本写入 `_spec/design_consumption_plan.json`。
- 覆盖前把上一版归档到 `_spec/archive/design_consumption_plan.{YYYYMMDD-HHmm}.json`。
- `surface_designs` 必须覆盖全部 surface，且不能存在 `gap` 才能交给 `wego-ux`。

落盘前自检：

- 所有业务信息均来自 `page_spec`，没有自行新增字段或流程。
