...
读取顺序固定为：`AGENTS.md` → 有效迭代与已确认 `prototype_brief` → `SKILL.md` → [设计决策方法](references/design-decisions.md) → `library-consumption.json` → 仅含明确范式的 `uikit-plan.json` → `components/index.json` → 本页命中的 Preview → 对应组件契约 → `colors_and_type.css` → [场景合同](references/scene-contract.md)`。`design-decisions.json` 仅是场景输出与已有场景修改时的辅助对照，不得作为设计前权威输入替代上述顺序。资产与权威来源按 [资产地图](references/library-map.md) 定位。只接受当前 Schema，旧输入直接失败。
