...
- 当 block 选中行式组件（如 cell）的连续组合时，`selected` 必须显式包含已注册的分组容器类（如 `.cell-group`），不发明任务级 `xxx-page__group` 自定义类替代；`implementation_constraints` 必须把"分组容器类强制使用"作为传递给 `wego-ux` 的硬约束
...
- `app` 不属于 UI Kit；固定宿主 App 模板的唯一来源是 `wego-ux/templates/host-shell.*`
- 组件必须优先复用已注册契约
...
- 当 `page_spec` 包含 `host_container + route_id` 时，`host-entry` 只负责输出宿主页入口/回填所需的 surface 依据；不要把固定宿主模板本身当作新的 pagePattern，也不要让下游参考后重画宿主 App
...
- 命中 `biz-rule-config` pagePattern 时，默认必须输出 `type: "full-screen-modal"`、`transition: "slide-up-enter, slide-down-exit"`、`overlay_level: "overlay"`、`covers_tab_bar: true`；只有先修改设计系统本体的 pagePattern 后，才允许输出其他打开方式
