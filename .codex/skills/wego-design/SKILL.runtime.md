...
- 已有 `page_spec`，要产出 `design_consumption_plan`
- 用户要判断页面范式、UI Kit、组件映射、页面打开方式、布局模式
- 用户明确要求“基于现有 `page_spec` 做设计系统消费”

不要误用场景:

- 还没有 `page_spec`，先回到 `wego-product`
- 当前目标是正式生成 `wego-app` 场景，转入 `wego-ux`
- 当前目标是改组件契约 / preview / UI Kit / 工作流规则，转入 `wego-uxsystem-iterate`
  ...
  "surface_designs": [
    {
      ...
      "matched_blueprint": "mobile-host-entry | object-management-list-page | generic-mobile-task-page | selection-list-page | null",
...
- 全局消费边界、资源复制边界、组件消费共性，以 `library-consumption.json` 为准
- 页面范式、surface 命中、`page_presentation`、`compositionConstraints`、`fallbackPageBlueprints`，以 `uikit-plan.json` 为准
- 新增 design 类规则时，默认回流到 `library-consumption.json` 或 `uikit-plan.json`，不要在本节重复展开正文
...
当 primary-task-page 的主任务是管理一组可新增、编辑、删除、启停或进入详情的业务对象，并且 `page_spec` 已声明列表必显字段、摘要字段、详情/编辑字段和操作字段时，场景类型必须标注为 `object-management-list-composition`。若没有精确 UI Kit/pagePattern，优先使用 `fallbackPageBlueprints[object-management-list-page]`，`matched_blueprint` 填 `object-management-list-page`，并在 `scene_fit_reason`、`component_mapping.reason` 或 `implementation_constraints` 中写清楚：

- 哪些字段留在列表，哪些进入详情/编辑页
- 列表项为什么采用横向主结构或纵向结构
- 对象识别图使用真实业务图片、对象缩略图还是低饱和占位
- 列表操作是外露还是收纳，以及收纳依据
- 导航栏新增入口是否使用图标+文字；若降级纯文字，必须说明原因
