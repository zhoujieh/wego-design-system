...
description: WeGo App 业务场景分层测试。业务场景 scene.js/scene.css/业务运行时 js 改动后做轻量回归，本地可验收版本做关键路径，终局验收前做全量走查；用于 App 走查、业务交互异常、bu.click 排查。
---

# WeGo App 业务场景自动化测试

## 触发条件
实现或修改 WeGo App 业务场景后回归；本地可验收版本、PR 更新、终局验收或业务交互异常排查时按层级加深测试。

## 固定流程
0. 定层级：本地小改=轻量冒烟；本地可验收版本/PR 更新=本地关键路径；终局验收=全量走查。
...
## 交付前检查清单
- 层级匹配：小改只需轻量冒烟；本地可验收版本或 PR 更新前完成本地关键路径；终局验收前完成全量走查。
- 证据链：本地可验收版本开始记录操作+DOM+必要 localStorage+时间戳；小改不强制逐项完整证据。
