...
⚠️ 禁止直接修改 `wego-app/scenes/{场景}/` 下的 scene.js 或 scene.css。
...
2. 问三个问题：
   ...
   a) 这次要改的内容，在 \_spec 的 page\_surfaces\[] 里有对应 surface 吗？
   → 没有 → 有"内容偏差"

   b) 这次要改的内容，如果是组件层面的变化，在对应 surface 的
   component\_mapping\[].selected 声明范围内吗？
   → 不在 → 有"组件偏差"

   c) 这次要改的内容，涉及展示方式/打开方式的变化，和
   \_spec 的 page\_presentation 一致吗？
   → 不一致 → 有"展示偏差"
...
| ---- | -------------------------------------- | ---------------------------------------------------- |
| 内容偏差 | page\_spec + design\_consumption\_plan | wego-product → wego-design → 回到 wego-ux → wego-tests |
| 组件偏差 | design\_consumption\_plan              | wego-design → 回到 wego-ux → wego-tests                |
| 展示偏差 | page\_spec + design\_consumption\_plan | wego-product → wego-design → 回到 wego-ux → wego-tests |
...
- 修复交互 BUG 但不改变交互模式
- 性能优化、代码结构重组
- 组件内已有变体之间的切换（前提是在 component\_mapping 范围内）
...
- 对象管理列表的操作可保留在列表内；1-2 个高频安全操作可外露，3 个及以上或包含危险/低频操作时优先使用已注册的更多操作/菜单/sheet/详情页承接。当前设计系统没有已注册气泡或菜单组件时，默认不临时发明 popover 类；只有 `design_consumption_plan` 明确写明用户要求列表内直接操作并允许场景级临时气泡菜单时，才可用当前场景业务作用域样式承接，且危险操作仍必须进入 dialog 二次确认

### component\_mapping 消费分派
