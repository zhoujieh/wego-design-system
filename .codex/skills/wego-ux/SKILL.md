---
name: wego-ux
description: 将 page_spec 和 design_consumption_plan 正式落成产品原型项目的技能。输出静态 HTML/CSS/JS 单预览壳多页面交互项目，不依赖前端框架，可在手机浏览器中直接预览；遵守 wego-design 的组件与规范边界。
---
...
使用**静态 HTML/CSS/JS** 单预览壳多页面交互项目结构，不引入任何前端框架依赖（无需 npm install、无需构建）。`index.html` 是唯一预览外壳和启动入口，不是唯一业务页面；完整流程必须拆成多个 screen、modal 或 fragment，不能把所有内容聚合成一个长页面。`pages/*.html` 若存在只能作为片段或迁移内容源。
...
`lib/` 下的 `fonts/`、`icons/`、`image/` 等子目录由 `assets/` 整体复制生成，不做任何筛选。项目可以扩展更多 `pages/*.html` 片段或内容源，也必须拆分完整交互流程的多个 screen/modal/fragment；所有可见交互从 `index.html` 壳内完成，不能低于这个最小结构。
...
9. 读取 `library-consumption.json` 中 `buildMobileAppPage.copyFiles`，按清单复制设计系统文件到 `lib/` 目录；`assets/` 目录必须**整体复制**到 `lib/`，不做任何子目录筛选或文件判断（不挑 `fonts/`、不挑 `icons/`、不挑 `app-center/`），确保所有资产完整可用；不在清单内的文件（如 scaffold.css、typography.css、css.json、specs/*.md、ui_kits/*、metadata.json）一律不复制
...
- “工作台”页默认用于承接业务工具入口；入口图标优先使用 `lib/icons/app-center/*.svg`；生成时已通过 `assets/` 整体复制把 `app-center/` 子目录带入 `lib/icons/app-center/`，不得只复制部分 SVG 或跳过 `app-center/` 子目录
...
| `full-screen-modal` | 壳内 overlay + 内联/template 内容 | overlay 预置在 `.phone-screen` 内，业务片段已在 `index.html` 内联或放入 `<template>` | 由 `page_presentation.transition` 映射，见下方转换表 | `slide-up-enter, slide-down-exit` |
...
3. 触发打开时，优先采用**内联方案**确保 `file://` 协议双击可用：
