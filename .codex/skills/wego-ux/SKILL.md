...
colors_and_type.css 定义了 --safe-area-top: env(safe-area-inset-top, 44px) 和 --safe-area-bottom-content: calc(40px + ...)。这些变量的 fallback 值在桌面浏览器中仍然生效，会导致导航栏上方多出空白、底部多出留白。

规则:
1. 原型项目的 page.css 中不引用 --safe-area-top 或 --safe-area-bottom-content
2. navbar 组件自带 transparent 背景，不需要额外 padding-top；真机 WebView 场景由宿主补齐
3. 页面底部 padding 使用固定间距（如 var(--spacer-16)），不使用 --safe-area-bottom-content
