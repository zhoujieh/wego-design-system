...
| 组件 | Slug | 状态 | 说明 |
|------|------|------|------|
| 卡片 | `card` | stable | 内容容器，支持白底/线框/填充三种表面；默认无阴影、无内边距 |
| 头像 | `avatar` | stable | 用户头像展示，支持图片、文字首字母等类型 |
| 标签 | `tag` | stable | 信息标签/标记，用于状态标识或筛选 |
| 角标 | `badge` | stable | 红点、数字、文字、移除四类角标；文字类默认绿色，可通过 CSS 变量自定义颜色与滚动宽度 |
...
| 开关 | `switch` | stable | 44x24px 布尔切换控件，适合设置项开启/关闭；通过伪元素补偿触控热区 |
...
- 只要组件本身带交互，展示时就直接并入原有变体、宿主结构或成组示例，不新增独立“交互演示”模块
