...
- 只要组件本身带交互，展示时就直接放回原变体、原宿主结构或原成组示例，不另起独立“交互演示”模块。
...
| `ui_kits/biz-settings/index.html` | 业务设置页面模式示例，展示从底部进入、落稳后全屏覆盖的模态设置页，以及 navbar/form/cell/checkbox/switch/radio 的组合使用。 |
