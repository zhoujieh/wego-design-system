...
- **触控**：紧凑 40px / 默认 44px / 舒适 48px，视觉不足时用伪元素扩展，详见 `specs/布局与间距规范.md` 操作热区章节。
...
- **已发布组件**：以 `components/index.json` 为准，共 18 个。
- **嵌入式组件**：`navbar-action-button` 已注册，但没有独立预览页；它作为 NavBar 右侧操作区使用，示例在 `preview/component-navbar.html`。
- **触控边界**：`counter` 保持紧凑视觉尺寸，通过 `counter__btn::before` 扩展热区，不通过增大布局尺寸满足触控要求。
...
- `assets/icons/` 中的 SVG 可作为已有资产参考，但不要在需要 iconfont 的组件里随意改用内联 SVG。
...
5. **触控可用**：可点击元素热区按场景分层——紧凑 40px、默认 44px、舒适 48px；视觉尺寸不足时用伪元素扩展，不改变布局。
