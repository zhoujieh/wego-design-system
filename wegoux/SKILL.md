...
- **组件迭代入口**：正式迭代组件时，优先使用仓库根目录 `.trae/skills/iterate-component/SKILL.md`，它定义了已注册组件迭代、新增组件发布、同步矩阵和强制守门流程。
...
- 迭代组件：`iterate-component`，先判断是已注册组件迭代还是新增组件发布，再按仓库根目录 `.trae/skills/iterate-component/` 下的标准流程执行。
