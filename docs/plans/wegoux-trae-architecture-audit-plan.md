...
> 历史归档文档（已停止维护）：本文保留当时的 `wegoux` 命名和比对语境，便于追溯背景；当前实际执行路径已经迁到 `.codex/skills/wego-design/`，请不要再把文中的旧路径当作现行权威路径。
...
> | 阶段 6：修复 UI Kit | **已完成** | 主 `app` 已移除 Lucide/CDN 图标、`phone-header`、`menu-item` 和 inline checkbox SVG，复用率实测 0.84 |
> | 阶段 7：同步文档和消费契约 | **已完成** | 本轮已同步 `ui_kits/app/quality-report.json`、本计划文档和 `metadata.json`；README/SKILL 现有边界仍有效 |
...
> 3. ✅ 全部 17 个预览页迁移到统一布局结构：通用组件用 Pattern A（`pv-header` + `pv-section` + `.row/.label`）；列表类用 Pattern B（`.page` + `.section-group/.section-gap`）。
> 4. ✅ chip → tag 重命名：`components/chip.json` → `components/tag.json`，`preview/component-chip.html` → `preview/component-tag.html`；组件注册表、uikit-plan、library-consumption、README、SKILL 同步更新。
> 5. ✅ 内联 SVG 替换：bottom-nav 4 个 tab SVG 替换为 `<img src="../assets/icons/...">`；tag 关闭/筛选 SVG 替换为 iconfont `<i class="wego-iconfont-s">`；cell radio 圆点 `#03C160` 硬编码修正为 `var(--color-brand)`。
> 6. ✅ 4 个预览页（switch/stack/radio/checkbox）中的 `.dark-strip`/`.demo-hint`/`.interactive-row` 样式从内联迁移到 `scaffold.css`。
> 7. ✅ checkbox checkmark 和 cell backspace minus 保留为 inline SVG（批准例外）。
> 8. ✅ `components.css` 通过 `node scripts/extract-components-css.mjs` 重新生成。
> 9. ✅ SVG 替换 4 级优先规则和迭代护栏写入 `specs/预览页脚手架规范.md`；SKILL.md 和 README.md 同步更新组件数量、脚手架引用、预览页规则。
>
> **验收结果**：
>
> - 17 个预览页全部引入 `scaffold.css`，无内联脚手架样式残留。
> - 标记块内无 page scaffold、无 demo-only 样式、无未解释 raw hex。
> - SVG 来源优先级：assets/icons/ → iconfont → 保留 inline（checkbox/cell 特定图标）。
> - 组件注册表 18 个稳定组件（chip→tag），无遗漏。
...
> **完成记录**（当前迭代）：
>
> 1. ✅ `ui_kits/app/index.html` 移除 Lucide CDN 和所有 `data-lucide` 调用，图标回到 `iconfont.css` 或 `assets/icons/`。
> 2. ✅ 用 `navbar` 组件结构替代 `phone-header`，状态栏保留为 navbar 内部展示区。
> 3. ✅ 用 `cell` 组件结构替代“我的”页面的 `menu-item` 列表行。
> 4. ✅ 用 `bottom-nav` 注册结构和 `assets/icons/tab-*.svg` 替代 Lucide Tab 图标，角标嵌回 `.bottom-nav__icon`。
> 5. ✅ 用 `counter` 替代详情页手写数量加减按钮。
> 6. ✅ 用 `image` 组件结构承载首页商品卡片和详情页商品图占位。
> 7. ✅ checkbox 勾选图标从 inline SVG 改为 `icon-gou16`。
> 8. ✅ 修复 `.section-gap` 同时承担页面容器和分隔高度的布局污染，分隔块改为 `.section-separator`。
> 9. ✅ `ui_kits/app/quality-report.json` 同步更新，复用率记录为 0.84，`inventedComponents` 只保留 Showcase 外壳。
>
> **验收结果**：
>
> - `ui_kits/app/index.html` 无 Lucide、`data-lucide`、`phone-header`、`menu-item`、`CHECK_SVG` 和 inline `<svg>` 残留。
> - 主 UI Kit 的发明组件只剩 `.uikit-shell`、`.phone-frame`、`.phone-screen`、`.phone-indicator` 这类不可复制的 Showcase 外壳。
...
> **完成记录**（当前迭代）：
>
> 1. ✅ `ui_kits/app/quality-report.json` 已按实际源码同步组件复用、发明组件和 warning。
> 2. ✅ 本计划文档已把阶段 6/7 从待办改为完成，并写入当前验收结果。
> 3. ✅ 本轮没有新增组件、改变注册表、改变 UI Kit 入口或改变下游读取路径，因此 `README.md`、`SKILL.md`、`library-consumption.json` 和 `uikit-plan.json` 的现有边界说明仍有效。
> 4. ✅ 正式迭代继续递增 `metadata.json.version`。
...
创建了移动端浅色预览脚手架 `scaffold.css`，只包含 body、pv-header、pv-section、row、label、dark-strip、demo-hint、interactive-row、page/section-group 等预览辅助，不包含真实组件样式，不引入暗色 IDE 视觉。所有 17 个预览页已统一引用。脚手架规范写入 `specs/预览页脚手架规范.md`。
...
本计划最初是审查文档；当前迭代已按阶段 6/7 落地到代码与质量报告。后续继续维护时仍应遵守：

- 不迁移 TRAE 的暗色、IDE、桌面组件或视觉语义。
- 不把 `.uikit-shell`、`.phone-frame`、`.phone-screen`、`.phone-indicator` 沉淀为通用组件。
- 改组件核心 CSS 时继续走 preview 标记区和 `components.css` 生成链路。
- 改 UI Kit 后同步 `quality-report.json`，正式迭代递增 `metadata.json.version`。
