...
## 当前进度与决策（2026-08-30 更新）

### MVP 已全部完成

- **M1 框架搭建**：权威源文件、主元素、Shadow DOM、FAB、工具条、Toast 全部完成
- **M2 元素选中**：走查模式开关、触摸/滑动区分、选中高亮、尺寸标注、CSS 选择器生成全部完成
- **M3 样式面板核心**：浮动面板、6 个分组（自动布局/字体/外观/填充/描边/投影）、样式读取与实时应用、变更记录全部完成
- **M4 颜色选择器**：Hex 输入、颜色面板、透明度、吸管全部完成
- **M5 配置列表+导出+持久化**：配置列表面板、单条删除、复制 Prompt、重置修改、localStorage 持久化、悬浮按钮红点指示全部完成
- **M6 同步与验证**：权威源同步、多场景验证、375px 适配全部完成

### 功能扩展期已完成（PR #140 迭代）

- ✅ 展开工具条自动激活走查模式
- ✅ ESC 逐级退出面板（颜色选择器 → 批注气泡 → Token 面板 → 样式面板 → 配置列表 → 子面板 → 走查模式 → 批注模式）
- ✅ 全部图标替换为 Phosphor Icons Light（1.5px 线宽，26 个图标）
- ✅ 批注系统（工具条批注模式、点击元素添加批注、气泡可编辑、持久化、配置列表合并展示、导出包含批注）
- ✅ 共享样式自动同步（修改公共样式时同类元素自动同步，配置列表合并为共享记录）
- ✅ 尺寸模式（固定/适应/填充）+ flex 多态尺寸兼容
- ✅ 顺序移动（自动布局内上下左右移动子元素，同类同步，边界置灰）
- ✅ 自动布局栏视觉优化（方向+顺序同一行、gap 输入框紧凑化、视觉统一）
- ✅ 设计系统 Token 选择（字号/颜色等可选择设计系统 Token）
- ✅ 守门验证脚本（validate-wego-design.mjs）
- ✅ 场景路由识别修复（2026-08-31：主 tab 切换清空 hash 导致场景混淆，无 hash 时从激活 panel 的 hostTab 映射 routeId）
- ✅ 复制 Prompt 改为跨场景汇总（2026-08-31：全部场景变更按场景分组输出，当前场景排第一）
- ✅ default 残留数据自动迁移（2026-08-31：逐条按 selector 定位 → host-tab 映射 routeId → 归并到正确场景，含去重）
...
- [ ] **CSS Grid 网格布局**（自动布局新增第三档"网格"，列数预设 1/2/3/4，列宽类型 Stretch/Fixed/Auto，九宫格场景优先验证）— 需求已确认，待实现
- [ ] 撤销/重做（Ctrl+Z）
- [ ] 键盘微调（方向键微调位置/尺寸）
- [ ] 测量模式（两元素间距测量）
- [ ] 网格辅助线（拖拽吸附）
...
### M1：框架搭建与入口（0.5 个工作单元）✅ 已完成

- [x] 创建 `walkthrough-tool.js` 权威源文件（IIFE 封装，挂载 `window.WegoApp.Walkthrough`）
- [x] 创建 `walkthrough-tool.css` 页面级样式
- [x] 实现 `wego-walkthrough` 主元素（Shadow DOM + 事件总线）
- [x] 实现 `wego-wt-fab` 悬浮按钮
- [x] 实现 `wego-wt-bottom-bar` 底部工具条（3 个按钮 + 更多菜单）
- [~] 实现 `wego-wt-banner` 走查横幅（用户决策：不需要，已取消）
- [x] 实现 `wego-wt-toast` 组件
- [x] 在 `index.html` 引入文件
- [x] 验证：悬浮按钮显示，点击展开工具条，按钮可点击，toast 正常

### M2：元素选中与高亮（0.5 个工作单元）✅ 已完成

- [x] 实现走查模式开关（事件拦截、触摸/滑动区分）
- [x] 实现 `wego-wt-overlay` 覆盖层
- [x] 实现触摸选中（2px 实线边框 + 尺寸标注）
- [x] 实现元素信息气泡（标签名+尺寸，位置自动适配）
- [x] 实现取消选中（触摸空白处 / ESC）
- [x] 实现 CSS 选择器生成（id / data-* / nth-child 路径链 + 唯一性验证）
- [x] 验证：走查模式下触摸选中元素，显示边框+气泡，滑动不误触，空白处取消选中

### M3：样式面板核心（1 个工作单元）✅ 已完成

- [x] 实现 `wego-wt-style-panel` 浮动面板（300px 宽，位置智能适配，纵向 section）
- [x] 实现面板元素信息行（标签名+选择器+尺寸）
- [x] 实现「自动布局」分组（布局方向、对齐矩阵、gap、padding、width/height）
- [x] 实现「字体」分组（font-size、font-weight、color、line-height、text-align）
- [x] 实现「外观」分组（opacity、border-radius）
- [~] 实现数值输入控件（通用组件，带步进按钮）（用户决策：不需要步进按钮，已取消）
- [x] 实现样式读取（getComputedStyle → 面板数据）和实时应用（element.style）
- [x] 实现变更记录（每次修改记录 oldValue/newValue）
- [x] 验证：选中元素后面板自动弹出，属性修改实时生效，变更被记录

### M4：颜色选择器 + 填充/描边/投影（0.5 个工作单元）✅ 已完成

- [x] 实现 `wego-wt-color-picker` 颜色选择器（原生 input color + Hex 输入 + 透明度滑块 + 吸管）
- [x] 实现「填充」分组（单层纯色：颜色按钮 + Hex + 透明度）
- [x] 实现「描边」分组（单层：颜色 + 宽度 + 位置）
- [x] 实现「投影」分组（单层：颜色 + 类型 + x/y/blur/spread）
- [x] 实现颜色解析（rgba ↔ Hex + 透明度）和投影解析（box-shadow 字符串解析）
- [x] 验证：颜色选择器弹出正常，填充/描边/投影修改实时生效

### M5：配置列表 + 复制 Prompt + 持久化（0.5 个工作单元）⏳ 红点待完成

- [x] 实现 `wego-wt-overview-panel` 配置列表面板（浮动面板，全部/配置 Tab）
- [x] 实现配置项展示（按元素分组、属性变更列表、单条删除、点击跳转选中）
- [x] 实现复制 Prompt（对齐 Liaison 格式 + 剪贴板复制 + toast）
- [x] 实现重置修改（清空当前场景变更 + toast）
- [x] 实现 localStorage 数据持久化（按场景路由隔离、自动保存/加载）
- [ ] 实现悬浮按钮红点指示（有未导出变更时）
- [x] 验证：配置列表展示正确，复制 Prompt 格式正确，刷新后数据恢复，重置正常

### M6：同步与验证（0.5 个工作单元）⏳ 多场景验证待完成

- [x] 权威源同步到 `wego-app/lib/js/walkthrough-tool.js` 和 `wego-app/lib/walkthrough-tool.css`
- [ ] 多场景验证（至少 3 个场景）
- [x] 375px 宽度下所有面板内容不溢出
- [x] 现有场景页面正常加载，无 JS 报错
- [~] 编写简要使用说明（用户决策：暂不需要）
