...
> 版本：v1.1（MVP，基于 Liaison v1.0.6 源码深度调研）
> 创建日期：2026-08-26
> 最后更新：2026-08-28
> 负责人：用户体验设计中心
> 状态：MVP 核心功能已完成，待打磨项进行中
> 参考源码：`docs/plan/research/liaison-extension-source/`
> 关联 PR：#140（走查工具 UX 优化 + Phosphor Icons 替换）

---

## 当前进度与决策（2026-08-28 更新）
...
- **M5 配置列表+导出+持久化**：配置列表面板、单条删除、复制 Prompt、重置修改、localStorage 持久化全部完成
- **M6 同步**：权威源同步到生成物完成
...
- ✅ 工具条增加批注模式按钮（与走查模式互斥）
- ✅ 批注模式下点击元素添加批注，气泡直接可编辑
- ✅ 自动保存输入内容，无保存/取消按钮
- ✅ 标记显示在元素左上角（橙色，左下角圆角小，对话气泡形态）
- ✅ 点击标记重新打开气泡，单例模式一次只显示一个
- ✅ 气泡位置智能适配（优先右侧，空间不够翻左侧/下方）
- ✅ 点击空白处/ESC 关闭气泡
- ✅ 批注持久化到 localStorage，按场景路由隔离
- ✅ 配置列表合并显示批注（与样式变更同组，橙色"备注"标签）
- ✅ 导出 Prompt 包含批注（**备注:** 字段，AI 可快速理解需求并定位元素）
- ✅ 重置修改同时清除批注
- ✅ 滚动/resize 时标记位置跟随元素变化
