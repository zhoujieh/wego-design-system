...
- 组件消费必须 Preview-first：先读本页命中组件 Preview，再读契约；禁止猜 Token、class、子元素或 modifier。场景代码中的组件 DOM 结构必须逐字复制 Preview 中对应变体的 HTML，包括节点层级、class 组合、修饰类顺序和可选子元素位置；禁止"理解大意后自己写"。
