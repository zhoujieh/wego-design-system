...
- 本地迭代默认先累计小问题和 checkpoint，完成一轮实现或反馈处理后只启动/复用本地预览，并通过临时 HTTPS 隧道返回一个可远程查看的本地预览链接；只有用户明确要求推送 PR、更新 PR、在线预览或多人协作需要远端同步时，才推送并创建/更新同一个 PR，随后等待部署完成，只返回 PR 在线预览链接并关闭对应本地预览与隧道；后续有新改动再重新进入本地预览循环。
...
- 临时验证用本地服务必须自动退出；本地迭代受管预览服务及其临时 HTTPS 隧道是唯一例外，并在切换 PR 在线预览、任务废弃、PR 关闭/合并或 worktree 清理时关闭。
