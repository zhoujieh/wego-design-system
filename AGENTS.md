...
- `wego-product` 完成 `prototype_brief` 草案后必须生成参考线框，再通过 `submit-brief --wireframe-generated-for-revision <scope_revision> → confirm-brief` 绑定当前范围并共同展示简报与线框；用户目标、入口、首要任务、主要操作、可见结果、必要状态和 `open_questions` 未齐备时先澄清，不得补造。
- 任何影响页面、入口、路径、状态或可见结果的简报修改都会使线框失效，必须重新生成后再提交确认；线框不替代简报确认，也不新增线框确认状态。无渲染能力时降级为文本分镜。
- 已确认 `prototype_brief` 即 `wego-design` 的设计与实现授权。先形成设计意图、精确范式/自主组合裁决、语义区域和页面布局合同，再选择组件；不得从组件清单反推布局、选择近似 UI Kit 或拼接多个 UI Kit。
