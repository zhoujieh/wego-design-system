...
- 收到业务页面的新建、优化、修复、验收反馈或复用请求时，进入 `wego-product` 或 `wego-design` 前必须先进入 `wego-github-delivery` 完成交付单元核对。核对必须覆盖全部 worktree、开放 PR、有效场景认领、未冻结迭代和本地预览记录；不得只依据当前分支或 `main` 判断页面、场景或交付单元不存在。命中已有单元时必须接手其分支、worktree 和已有 PR；只有查无匹配且无冲突时才能新建迭代或分支。
- 只暂存本次任务的显式路径，不执行 `git add -A`，不强推已有远端分支。

<!-- rule-id: delivery-must-require-explicit-submission -->
- 新任务、首次实现和后续小问题调整默认处于**本地迭代中**。只创建或复用任务分支与 worktree，允许轻量验证、真实浏览器检查和按一组相关改动创建本地 checkpoint commit；禁止自动推送、创建或更新 PR、等待 GitHub Actions 或刷新在线预览。
- 只有用户明确要求“提交”“推送”“创建/更新 PR”“发在线预览”，或明确说明“这一轮改完了，可以验收”时，才进入**正式验收中**。用户说“改好了”“继续”“再调整一下”等普通反馈不构成提交授权，Agent 不得自行判断任务已完成而推送。
- 已有开放 PR 后继续收到小问题，也必须先在同一 worktree 本地累计完成；只有用户再次明确要求更新 PR，才批量推送一次。多人或多 Agent 确有远端协调需要时，可说明原因后仅推送最小认领提交，但不得因此创建 PR 或进入正式验收。

<!-- rule-id: delivery-status-must-be-reported -->
- 每轮结果必须标明交付状态：本地阶段使用 `当前状态：本地迭代中（未推送）`；正式阶段使用 `当前状态：正式验收中（PR #<编号>）`。不得用“已完成”模糊代替状态。

<!-- rule-id: design-system-preview-delivery-uses-local-address -->
- 本地迭代阶段，有可视内容时只提供已核实的本地 HTTP 预览链接；纯工作流、守卫或文档维护没有可视入口时只报告验证结果，不强行生成预览。进入正式验收后，业务原型同时提供本地 HTTP 与当前 PR 独立在线预览，两个链接进入同一 routeId 或目标入口；设计系统组件、Token、Preview 或 UI Kit 更新仍只提供本地 HTTP 预览，不返回 GitHub Pages 预览链接。
- GitHub Pages 的 PR 预览只用于正式业务原型验收；本地迭代不等待 Pages，设计系统预览也不在 Pages 入口验收。`main` 的 GitHub Pages 链接只展示已经验收并合并的稳定业务版本。

<!-- rule-id: local-preview-branch-must-match-open-pr -->
- 本地迭代预览前，必须核对承载服务的 worktree 当前分支与任务分支一致，并确认目标内容包含本次改动；正式验收未合并 PR 时，还必须核对该分支与 PR head 一致。不一致时不得返回本地地址。等待反馈期间不得切换该 worktree 到无关分支；无关任务使用独立 worktree。

<!-- rule-id: acceptance-preview-server-lifecycle -->
- 本地预览服务从当前交付单元 worktree 根目录启动，仅绑定 `127.0.0.1`；端口、PID、worktree、分支、阶段（`local-iteration` 或 `formal-review`）、可空 PR 编号和目标入口记录在 `.tasks/preview-servers/`，记录不得提交。
- 回复结束、对话结束或等待用户查看期间不得关闭仍有效的本地迭代或验收服务。从本地迭代进入正式验收时复用原服务并补充 PR 信息，不为同一任务重复启动服务。
- 用户明确验收通过、明确废弃任务、PR 关闭或合并、对应分支/worktree 即将清理，或服务被同一交付单元的新服务替换时，必须停止服务并删除记录。新任务开始时清理进程失效、worktree 不存在、分支不匹配，或已关联且 PR 已关闭/合并的残留服务；不得把“尚未创建 PR”视为残留，也不得误停其它有效任务服务。
- 用户明确验收通过后，先停止对应本地验收服务，再同步最新 `main`、解决冲突并完成验证，最后合并到 `main`；不得根据“推送”或“创建 PR”推断用户已经同意合并。
- PR 合并不得绕过分支保护、必要检查或冲突处理；合并失败时停止并修复问题。
- 正式验收创建或更新 PR 后，GitHub Actions 将业务原型分支发布到 `previews/pr-{PR编号}/`；PR 后续更新覆盖同一预览地址，PR 关闭或合并后清理对应预览目录。设计系统变更可继续触发该动作，但不得将它作为组件或 UI Kit 的验收入口。
- 允许的短期临时目录为 `.uploads/`、`.tasks/`、`output/` 和 `.playwright-cli/`；临时产物不得提交，任务收口时清理无用文件，需长期保留的资源必须迁移到正式目录。仍在本地迭代或等待用户验收的服务记录不是无用临时产物。
- 按需执行 `node scripts/cleanup-task-artifacts.mjs clean` 清理任务临时产物；不得删除仍在本地迭代或等待验收的服务记录。

### 多人多 Agent 并发协作

多个人各自驱动自己的 Agent 会话、并发迭代同一仓库。Agent 之间不对话，协调只发生在仓库层面：谁的 Agent 碰了什么，由仓库状态体现，别的 Agent 来读。

- **分支与 PR**：一个交付单元固定一个 `feature/<owner>-<task>` 分支和 worktree；首次开始任务默认不创建 PR。进入正式验收后，一个交付单元最多对应一个开放 PR。业务原型正式验收通过本地 HTTP 与独立 PR 在线双预览，设计系统组件、Token、Preview 或 UI Kit 只通过对应本地 HTTP 预览；只有用户明确验收通过后才合并到 `main`。PR 合并或关闭后先停止本地验收服务，再默认删除本地和远端分支；只有 `keep-branch` 标签可例外保留分支。

<!-- rule-id: agent-must-pull-before-task-start -->
- **开工前先拉取**：每次新会话/新任务开始前执行 `git pull --rebase origin main`，确保基于最新 `main`；首次获得提交授权、准备创建 PR 前再次 rebase 到最新 `main` 并解决冲突。已有开放 PR 的分支同步 `main` 改用 merge，不用 rebase（详见 `wego-github-delivery` 交付规则）。
...
- **场景认领（强制前置）**：开工前必须在 `claims/<agent-id>.json` 认领本次负责的场景目录并记录当前 `branch`，再运行 `node scripts/validate-claims.mjs` 确认无冲突；不判断是否并发，单人开发也必须认领。认领期间场景目录由该 Agent 独占，他人不得修改。完成、废弃或关闭对应交付单元时把 `status` 改为 `released`/`done` 释放；CI 在正式 PR 阶段核对所有场景目录变更均有对应认领。确有多人远端协调需要时，可只推送最小认领提交暴露占用状态，但不得自动创建 PR。
...
- 本地迭代中的普通改动运行与范围相称的轻量检查和真实浏览器验证。
- 获得提交授权后，普通改动运行 `node scripts/validate-wego-design.mjs`。
- 设计系统或工作流正式提交前运行 `node scripts/validate-wego-design.mjs --scope=system --strict`。
- 正式合并前按需运行 `node scripts/validate-wego-design.mjs --scope=full --strict`。

<!-- rule-id: local-server-must-auto-exit -->
临时浏览器验证、自动化测试或一次性检查使用的本地服务必须自动退出，任务结束前确认没有遗留监听进程。`acceptance-preview-server-lifecycle` 定义的本地迭代与用户验收预览服务是唯一例外：它在等待用户继续反馈或验收期间保持运行，并在验收完成、任务废弃、PR 关闭或合并、worktree 清理或服务替换时由 Agent 负责关闭。
